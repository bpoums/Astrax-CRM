...
- Deployed to **Cloudflare Workers**, not Vercel:
  ...
  npx wrangler deploy
  ```
  **`wrangler.json` at the repo root pins `name` and `compatibility_date`** —
  read it before changing anything about deployment. It exists because both
  were previously derived per-build and both failed silently (2026-09-17):
  - `name` was auto-generated by Nitro **from the git remote**
    (`bpoums/closerform` → `bpoums-closerform`). Renaming the GitHub repo to
    `bpoums/Astrax-CRM` changed it to `bpoums-astrax-crm`, so `wrangler deploy`
    reported **success while publishing to a different worker** and the live
    site never changed. Nothing in the output said so.
  - `compatibility_date` was stamped with **today's date** every build, and
    Cloudflare rejects a future date — which is why the flag used to be
    required and why deploys broke after ~7pm PKT.

  So the extra flag is no longer needed. `.output/server/wrangler.json` is
  **generated** — never hand-edit it; change the root `wrangler.json` instead,
  and do not put `main` or `assets` there (Nitro overrides both and warns).
- Live at <https://bpoums-closerform.umsvalidation.workers.dev>. Changing the
  worker `name` moves this address — see the three places below that must move
  with it.
